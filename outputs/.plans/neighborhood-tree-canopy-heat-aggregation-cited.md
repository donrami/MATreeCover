...
3. **For individual- and population-level exposure studies, the standard is buffer-based canopy/greenery around residences, with population (or building/address) weighting** — e.g., Song et al. 2024 (*Environment International* 191:108950) used population-weighted greenspace exposure at buffers of 200–4000 m; Mears et al. 2020 (*Health & Place* 62:102284) averaged trees in 100 m buffers per residence within LSOAs (as described in Garrett et al. 2025); Garrett et al. 2025 (*Int J Health Geogr* 24:37) averaged land-cover % in 300 m postcode buffers weighted by domestic delivery addresses; Browning et al. 2024 (*Sci Total Environ* 907:167739) measure the "3-30-300" rule (3 trees visible from home, 30% neighborhood canopy, green space within 300 m) at address level.

4. **The two aggregation methods produce materially different numbers and different rankings**, with documented magnitudes: built-up (non-green) exposure is 8–10% higher when measured with address-weighted buffers than with LSOA area averages (Garrett et al. 2025); population-weighted canopy in Baltimore fell from 47–51% in the least-deprived decile to 12–14% in the most deprived (3.4–4.1× disparity; Ibebuchi & Abu 2026, *World* 7(1):6); a density decline of 10,000 persons/km²/decade is associated with a population-weighted surface-UHI-exposure trend ≈0.5 K/decade slower than the area-average trend (Huang, Stone Jr., Guan & Liang 2025, *Scientific Reports* 15:13860); within-city greenspace *exposure* inequality (Gini coefficient over residents' exposure, 0 = perfect equality) is ≈2× higher in Global South cities (0.47) than in Global North cities (0.24), and provision's unique contribution to between-city variation in exposure inequality is only ~22% (21.93% of variance; Chen et al. 2022, *Nature Communications* 13:4636).

5. **Presenting one metric as the other is an ecological-fallacy-class error.** The clearest statement in the literature: "measures of provision (in total or per capita) are often simplistically equated to actual exposure on the assumption that the population is evenly exposed to greenspace over time and space. […] Therefore, aggregated unit measures of greenspace total supply or per capita supply produce an ecological fallacy—biased inferences of individual patterns from aggregated data" (Chen et al. 2022; ellipsis marks omitted intervening sentences). Relatedly, district-level % canopy is inflated by parks and forest patches; per Grace et al. (2025, *Land* 14(8):1686), park cool-island *air-temperature* effects are small for most parks (surface temperatures run 10–15 °C cooler but street-level air temperatures barely differ), so counting park canopy in a district "cooling" metric overstates the mitigation residents actually experience — while building-weighted buffers can in turn under-count the district parks that do provide some cooling. Metric choice can even reverse the sign or significance of heat–health effect estimates (Aboudrar-Méda & Falchetta 2026, *Environ. Res.: Infrastruct. Sustain.* 6(2):021001; Song et al. 2023, *Environ Health Perspect* 131(9):097007).

6. **The field is converging on reporting both** (provision + exposure) and on multi-scale sensitivity analysis (Zhang & Tan 2019, IJERPH 16(4):578; *Lancet Planetary Health* 9(11), 2025 buffer guidance; Modifiable Areal Unit Problem (MAUP) analyses, e.g., Zhang et al. 2026, *Landscape Ecology* 41(5):77; Deng, Liu, Feng & Xiong 2024, *Sustainable Cities and Society* 114:105747).
...
BWCanopy(D) = Σ_b w_b · c(b) / Σ_b w_b
with weights w_b = footprint area (or building count, or estimated residents)
```
...
- As r grows beyond the district extent, every building's buffer contains the whole district, so c(b) = CanopyShare(D) for all b and BWB collapses to the district share regardless of weights; as r → 0, BWB measures canopy immediately adjacent to buildings.

**Two orthogonal axes.** Weighting basis (area / population / buildings) and spatial support (administrative polygon / buffer) are independent choices, giving four metric families:

```mermaid
flowchart TD
    A[Canopy raster / land-cover map] --> B{Weighting basis}
    B -->|none| C{Support}
    B -->|population / addresses / buildings| D{Support}
    C -->|district polygon| E[AWD: area-weighted district share]
    C -->|buffer around point/building| F[area-weighted buffer mean]
    D -->|district polygon| G[population-weighted polygon mean e.g., Baltimore CBG means]
    D -->|buffer around buildings| H[BWB: building/population-weighted buffer mean]
    E --> I[Standard in HVIs and UHI mapping]
    G --> J[Standard in equity audits]
    H --> K[Standard in exposure epidemiology]
```

Note that many "population-weighted" studies operate on the **polygon** axis (population-weighted zonal means, e.g., Ibebuchi & Abu 2026), while others operate on the **buffer** axis (e.g., Garrett et al. 2025); conflating the two axes is as problematic as conflating area- and population-weighting.
...
- Wolf & McGregor 2013 (London, *Weather and Climate Extremes* 1:59–68): 9 census-based proxies for 4,765 census districts, PCA. Notably this HVI contained **no green-space component** — an example of the heterogeneity in whether canopy is included at all.
- Operational HVIs use the same geometry: **NYC DOHMH HVI** scores neighborhoods 1–5 using surface temperature, **green space (tree/grass/shrub cover % of the neighborhood)**, AC, and income (the HVI explorer page reports a median neighborhood green-space share of ≈25%); **DC Heat Exposure Index** aggregates tree canopy cover and impervious surface **averaged by census tract**; **NSW Greater Sydney** HVI + canopy cover reported at suburb/LGA level from SA1 data.
...
- Scale effects are documented and material: canopy–temperature relationships are scale-dependent (Ziter et al. 2019 PNAS — canopy effects depend on spatial scale and interact with impervious surfaces; Ralls, Polyakov & Shandas 2024, *Land* 13(11):1741 — effects vary across 10–90 m radii; Grace et al. 2025, *Land* 14(8):1686 — park cool-island effects small in air temperature for all but the largest parks). MAUP studies show LST driver rankings and effect sizes change with grid size (Zhang et al. 2026, *Landscape Ecology* 41(5):77, Zhengzhou: optimal unit 3×3 km; green-space ratio q = 0.641; Deng, Liu, Feng & Xiong 2024, *Sustainable Cities and Society* 114:105747: human-related factors (buildings, residents' work/life patterns) show higher LST scale sensitivity than natural factors across 87 cities and 12 spatial scales (per the paper's abstract).
...
- **Baltimore (Ibebuchi & Abu 2026, *World* 7(1):6):** population-weighted canopy fell from 47–51% (least-deprived decile) to **12–14%** (most-deprived) — a **3.4–4.1× disparity** (the paper's Discussion says 13–15%; Results/Conclusions say 12–14%). Note this is a *population-weighted polygon* metric: canopy is an area-zonal mean per census block group, then population-weighted for decile means and a weighted-Gini inequality index — not a buffer metric. The separate block-group canopy–deprivation Spearman correlation (ρ ≈ −0.51) is an unweighted, polygon-level statistic, not the same quantity measured two ways.
- **Global cities (Chen et al. 2022, *Nature Communications* 13:4636):** within-city greenspace exposure inequality (Gini) is ≈2× higher in Global South cities (0.47) than Global North cities (0.24). In variance decomposition, provision (coverage rate) uniquely explains 21.93% of between-city variation in exposure inequality, configuration (edge density) uniquely 3.68%, and provision × configuration jointly 53.42% — i.e., exposure inequality is driven mostly by *how much* greenery exists and how it co-varies with population placement, not by configuration alone. The paper's coverage-vs-exposure examples (e.g., New York City: ≈58% greenspace coverage vs ≈26% population-weighted exposure) illustrate the supply–exposure gap directly.
- **Global SUHI trends (Huang et al. 2025, *Sci Rep* 15:13860):** a decrease of 10,000 persons/km²/decade in population-weighted density is associated with a population-weighted SUHI-extreme trend ≈0.5 K/decade slower than the area-average trend; the gap widens as population-weighted density declines (74.7% of declining-density cities showed slower population-weighted than area-average trends, vs 56.8% of densifying cities, and the regression intercept is negative, so on average population-weighted exposure rose more slowly). In densifying cities the population-weighted vs area-average difference is inconsistent — so "area averages understate densifying cities" is **not** a supported general claim.
- **Helsinki (Laatikainen, Hasanzadeh & Kyttä 2018, *Int J Health Geogr* 17:29):** compared administrative units, 500 m residential buffers, home-range models, and an individualized residential exposure model for built-environment–wellbeing associations; the paper's abstract reports that a positive association between green space and wellbeing was found **only** with the individualized exposure model, and that the four units differed in size, shape, and what contextual measures they capture.
...
- District % canopy counts parks, forests, riparian corridors, and institutional green land that are far from residences; the building-weighted buffer down-weights them. Two nuances cut both ways:
  ...
  - Conversely, tree clusters can have *non-local* microclimatic effects: in a Singapore park-and-neighborhood microclimate simulation, unshaded zones between trees heated up by up to ~25% relative to a no-trees reference due to wind blocking and added humidity from transpiration (Derome, Nevers, Kubilay & Carmeliet, ICUC12 conference contribution, https://meetingorganizer.copernicus.org/ICUC12/ICUC12-509.html?pdf=).
...
- Building-weighted buffers can *under*-count district parks whose cooling extends beyond the buffer radius: park cooling reaches nearby blocks (e.g., 2SFCA-based analyses of who actually receives park cooling benefits — "Cooling inequity: investigating environmental equity in residents' access to cooling services from urban parks using an improved 2SFCA model", *Urban Climate*, https://www.sciencedirect.com/science/article/abs/pii/S2212095525002913), and buffers can mis-rank districts when building footprints misrepresent residential population (offices/warehouses with large footprints but no residents). Address/delivery-point weighting (Garrett et al. 2025) and gridded population (Chen et al. 2022; Zhong, Zhao, Ren, Teng & Zhang 2024, *Building Simulation* 17(11):1933–1949) are the recommended fixes.
- Edge effects: administrative-boundary averaging ignores canopy just across the boundary; buffers handle this but introduce radius choice (UGCoP; Kwan 2012, *Ann Assoc Am Geogr* 102(5):958–968; *Lancet Planetary Health* 9(11), 2025 guidance; Mears et al. 2020). Relatedly, park-access and greenery metrics computed at aggregated units are not statistical summaries of their sub-units and can mislead when the aggregation unit is far larger than residents' walking range ("Influence of spatial scale on the study of access fairness of urban park green space", *Frontiers in Environmental Science* 2022, 10.3389/fenvs.2022.1030796).
...
- Several studies find canopy outperforms total vegetation or grass for health-related outcomes (mental health in Singapore, Zhang & Tan 2019; wellbeing in NYC, Reid, Clougherty, Shmool & Kubzansky 2017, IJERPH 14(11):1411), and canopy dominates daytime surface cooling in scale-dependent analyses (Ziter et al. 2019) — but heat-specific health evidence for canopy-vs-vegetation is still mostly indirect.
...
- HVI predictive validity is weak and sensitive to unit choice and indicator selection (Niu et al. 2021).
...
- NYC DOHMH Heat Vulnerability Index (green space component) — https://a816-dohbesp.nyc.gov/IndicatorPublic/data-features/hvi/ **[OK]** (the explorer page reports a median neighborhood green-space share of ≈25%, cited in §3)
...
- Deng, H.; Liu, K.; Feng, J.; Xiong, Y. (2024) "Tackling the modifiable areal unit problem: enhancing urban sustainability through improved land surface temperature and its influencing factors analysis", *Sustainable Cities and Society* 114:105747 — https://www.sciencedirect.com/science/article/abs/pii/S2210670724005729 **[BLOCKED]** (ScienceDirect bot wall; metadata confirmed via CrossRef; "87 cities, 12 spatial scales" and the human-vs-natural-factor scale-sensitivity direction cited in §4 are from the paper's abstract)
...
- Ibebuchi, C.C.; Abu, I.-O. (2026) "Threefold Environmental Inequality: Canopy Cover, Deprivation, and Cancer-Risk Burdens Across Baltimore Neighborhoods", *World* 7(1):6 — https://www.mdpi.com/2673-4060/7/1/6 **[OK]** (⚠ resolved: journal is **World** (ISSN 2673-4060), not *Urban Science*; authors/year; 47–51%→12–14%, 3.4–4.1×, ρ≈−0.51 confirmed; the paper's Discussion says 13–15%)
...
- Laatikainen, T.E.; Hasanzadeh, K.; Kyttä, M. (2018) "Capturing exposure in environmental health research: challenges and opportunities of different activity space models", *Int J Health Geogr* 17:29 — https://ij-healthgeographics.biomedcentral.com/counter/pdf/10.1186/s12942-018-0149-5.pdf **[OK]** (⚠ resolved: authors; the green-space–wellbeing result quoted in §5.2 is from the paper's abstract)
...
- Derome, D.; Nevers, C.; Kubilay, A.; Carmeliet, J. "Assessment of heat exposure and cooling efficiency by trees in urban microclimate analysis" (ICUC12 conference contribution, Singapore park simulation) — https://meetingorganizer.copernicus.org/ICUC12/ICUC12-509.html?pdf= **[OK]** (conference contribution; supports the §6.2 non-local tree-cluster effects point)
- "Cooling inequity: investigating environmental equity in residents' access to cooling services from urban parks using an improved 2FSCA model", *Urban Climate* — https://www.sciencedirect.com/science/article/abs/pii/S2212095525002913 **[OK]** (supports the §6.4 point that park cooling extends beyond park boundaries)
- "Influence of spatial scale on the study of access fairness of urban park green space", *Frontiers in Environmental Science* (2022), DOI 10.3389/fenvs.2022.1030796 — https://www.frontiersin.org/journals/environmental-science/articles/10.3389/fenvs.2022.1030796/full **[OK]** (aggregation-unit accessibility is not the statistical summary of its sub-units)
...
- **UNVERIFIED (kept and flagged, not silently dropped):** National-HVI "55,267 tracts" figure; Lancet Planetary Health 2025 full author list. (Items previously flagged — NYC HVI 25%, Deng et al. "87 cities/12 scales" direction, Graham et al. ~15×, Laatikainen significance, ICUC12, Boston CEE 2025, 2SFCA — were subsequently re-attributed to their source pages/abstracts or replaced with sourced entries during the review-fix pass; see the plan ledger.)
