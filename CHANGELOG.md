## 2026-08-09 — Ko-fi donation button (017)
- Surface footer gains the owner's Ko-fi donation button (`src/site/index.html`): plain external link (no script, no tracking, FR-008) opening `https://ko-fi.com/M4Q624RYOV` in a new tab with `rel="noopener"`, official CDN image (`https://storage.ko-fi.com/cdn/kofi3.png?v=6`, width/height attributes reserve 143×36 to avoid CLS), accessible name „Unterstützen" (FR-006, owner-chosen short German label). Placement revision (clarification session 2026-08-09): the header was judged too prominent — the banner lives centered in `.surface-footer` (last child of `.surface-body`, sticky bottom) and is visible only when the surface is expanded; it is hidden when collapsed (`display: none`, expanded-only contract FR-001). The footer's bottom radius matches the panel's 8-px corners on desktop; mobile touch target ≥ 44×44 px with ≥ 8 px spacing; the header-packing image-shrink fallbacks are deleted.
- CSP note: `img-src` gains exactly one origin (`https://storage.ko-fi.com`) in lockstep across the meta tag, the Worker header, and the feature-015 contract; every other directive stays byte-identical (FR-009). Placement-agnostic, unchanged by the revision.
- Verification: `tests/acceptance/test_ko_fi.py` (footer position + exact alt + hidden-when-collapsed CSS + deleted-rule absence, CSP lockstep), ko-fi scenario in `scripts/smoke-verify.mjs` (footer visible at 768/1280/1920 px, collapse hides it, click opens the exact URL in a new tab, map page stays open), manual checklist `tests/frontend/smoke_ko_fi.md`. Gates (`check-public`/`check-layout`/`check-or005`/`check-history`), full suite (188 passed), interaction budgets (medians 13–14 ms), and parity render all pass; the button is purely additive (FR-013).
