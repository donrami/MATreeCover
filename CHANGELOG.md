...
- Visible crawlable content: `#about` section (story, method, data sources, accuracy disclaimer) in the raw HTML outside hidden containers; the first-visit story modal now shows over the map without scrolling and holds only an attribution-page link (new tab), holding no second DOM copy (feature 007 preserved).
