...
- Surface header gains the owner's Ko-fi donation button (`src/site/index.html`): plain external link (no script, no tracking, FR-008) opening `https://ko-fi.com/M4Q624RYOV` in a new tab with `rel="noopener"`, official CDN image (`https://storage.ko-fi.com/cdn/kofi3.png?v=6`, width/height attributes reserve 143×36 to avoid CLS), German alt „Baumfläche auf Ko-fi unterstützen". Visible in both surface states on the desktop panel and the mobile bottom-sheet header: touch target ≥ 44×44 px, ≥ 8 px spacing, at most 2 header rows at ≥ 320 px; 32-px image fallback below 350 px and on the desktop title row (R-4 packing).
- CSP note: `img-src` gains exactly one origin (`https://storage.ko-fi.com`) in lockstep across the meta tag, the Worker header, and the feature-015 contract; every other directive stays byte-identical (FR-009).
- Verification: new `tests/acceptance/test_ko_fi.py` (markup, CSP lockstep, touch-target group membership, focus rule), ko-fi scenario in `scripts/smoke-verify.mjs` (button visible at 768/1280/1920 px, click opens the exact URL in a new tab, map page stays open), manual checklist `tests/frontend/smoke_ko_fi.md`. Gates (`check-public`/`check-layout`/`check-or005`/`check-history`), full suite (184 passed), interaction budgets (medians 13–14 ms), and parity render all pass; the button is purely additive (FR-013).
