...
**Purpose**: Close gaps found by the convergence pass (2026-08-09): the feature 016 bundle is committed but NOT deployed — the live surface `https://abu-hamad.de/map/` still serves the pre-feature state (title `Baumfläche`, no description/canonical/OG, `/map/robots.txt`/`/map/sitemap.xml`/`/map/og-image.png` all 404), so the user-story independent tests, SC-002/005/006/010, and the report's after-state claims cannot reproduce yet.

- [X] T036 Deploy the feature 016 bundle to the live site (established deploy flow, `scripts/deploy-cf.sh` per specs/005-host-on-personal-domain/contracts/deployment.md; requires owner `deploy-cf.env` / `MATREECOVER_*` secrets) and verify the deployed surface per US1/US2/US3 Independent Tests: `https://abu-hamad.de/map/` raw HTML carries the descriptive 30–60-char title, 100–160-char description, canonical, and full OG/Twitter set; `/map/robots.txt`, `/map/sitemap.xml`, `/map/og-image.png` each return 200 with correct content types and the same security headers as other Worker responses (US1 Independent Test, US2/AC2, US3 Independent Test, SC-002/SC-005/SC-006/SC-010, plan incremental delivery — `missing`)
- [X] T037 After deployment, re-run every check in the Verification section of `verification/seo-assessment-2026-08-09.md` against the live site (raw HTML fetch, robots/sitemap 200 + content types, og:image 200 / PNG / 1200×630 / < 1 MB, Rich Results Test zero errors under the locked CSP) and update the report's after-state table and verification output so each stated result reproduces; record quickstart.md Q1–Q8 validation results per FR-001/SC-001/SC-007 and T033 (`partial`)
- [X] T038 Reconcile the interaction-count discrepancy in `verification/seo-assessment-2026-08-09.md`: the decisions summary states "Eight interactions at or under 2 s" while the Verification section records "5 interactions" — align the wording with SC-008's 8-interaction budget and what `scripts/perf-measure.sh` actually probes (SC-008, US4/Independent Test — `partial`)
