...
- Q: What happens to the first-visit story modal (feature 007) once the story text becomes visible on the page? → A: Single source — the visible section is the canonical copy and the modal presents a link to the attribution page (opening in a new tab) instead of holding a second copy; no duplicate story text in the DOM.
...
- Q: When the first-visit modal opens, what should happen to get the user to the story section? → A: No scrolling at all — restore the previous overlay behavior: the modal shows over the map and contains only a link to the attribution page, which opens separately (new tab). The visible `#about` section stays the single story copy (FR-006); the auto-scroll-to-`#about` and the scroll-back-on-close are removed.
...
**Independent Test**: Fetch the raw HTML of `https://abu-hamad.de/map/` and assert: descriptive title (30–60 chars), meta description (100–160 chars), canonical tag matching the canonical URL, and that the informative text is present in raw HTML without executing JavaScript or opening the story dialog.
...
3. **Given** the map page, **When** its raw HTML is fetched without JavaScript execution, **Then** at least 80 % of the informative words (story, method, sources, disclaimer) appear in the raw HTML body, outside hidden containers.
...
- **Hidden vs. visible copy duplication**: resolved (Clarifications 2026-08-09) — single source. The visible section is the canonical copy of the story text; the first-visit story modal (feature 007) presents that same content instead of holding a second copy. Exactly one story text location exists in the DOM, so the SC-004 word count is unambiguous (counted once).
...
- **First-visit story modal (feature 007)**: preserved (Clarifications 2026-08-09). The modal shows over the map with no scrolling: it contains only a link to the attribution page, which opens separately (new tab); the dismiss-persistence behavior is unchanged. The visible `#about` section is the single story copy. Its smoke test (smoke_us7.md) reflects the interaction.
...
- **FR-006**: The primary informative content (story, method, data sources, accuracy disclaimer) MUST be visible in the initial HTML without user interaction and without JavaScript execution; at least 80 % of its words MUST be outside hidden containers. The story text MUST exist in exactly one DOM location (Clarifications 2026-08-09): the visible section is the canonical copy and the first-visit story modal (feature 007) presents that same content without duplicating it.
...
- **SC-004**: At least 80 % of the informative text words are present in raw HTML outside hidden containers, without JavaScript execution or interaction.
...
- **SC-010**: The map page's indexable HTML (raw fetch) is self-sufficient: a crawler that cannot run JavaScript still encounters the page's full informative message.
