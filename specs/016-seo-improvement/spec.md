...
- Q: What happens to the first-visit story modal (feature 007) once the story text becomes visible on the page? → A: Single source — the visible section is the canonical copy and the modal opens/presents that same content; no duplicate story text in the DOM.
...
- **First-visit story modal (feature 007)**: preserved (Clarifications 2026-08-09). The modal now opens or scrolls to the visible story section instead of holding its own hidden copy; the dismiss-persistence behavior is unchanged. Its smoke test (smoke_us7.md) is updated to the new interaction.
...
- **SC-002**: Every page resolves to exactly one indexable URL: the canonical tag matches the canonical URL, `/map/` and `/map/index.html` do not present as duplicate content (the literal path already 307-redirects — research.md D1), and `www`/no-trailing-slash variants 301 to the canonical.
